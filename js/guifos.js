...
const iconCamera = document.getElementById('camera-button');
    ...
    navigator.mediaDevices.getUserMedia({
        ...
        .then(function (stream) {
            ...
            record.addEventListener('click', () => {
                ...
                recording = !recording;
                iconCamera.src = './assets/recording.svg';
...
function uploadGif(gif) {
    ...
    fetch(apiUpload + '?api_key=' + apiKey, {
        ...
        if (res.status != 200) {
            uploadMessage.innerHTML = '<h3 class="font-family2">Hubo un error subiendo tu Guifo</h3>'
        }
        return res.json();
    }).then(data => {
    ...
    fetch(apiBaseUrl + id + '?api_key=' + apiKey, {
        ...
        .then((res) => {
            ...
            embed.addEventListener('click', async () => {
                ...
                copyModal.classList.remove('hidden');
                copyModal.innerHTML = 'Código embebido con éxito!';
...
window.addEventListener('load', () => {
    ...
    localGifs.forEach(item => {
        ...
        containerImg.classList.add('container-results-gifs')
        img.src = item;
        img.classList.add('results-gifs');
        document.getElementById('results').appendChild(containerImg);
    });
});
...
sailorDay.addEventListener('click', () => {
    ...
    arrow.style.border = 'dotted 4px transparent';
    iconCamera.src = './assets/camera.svg';
});
...
sailorNight.addEventListener('click', () => {
    ...
    arrow.style.border = 'dotted 4px white'
    iconCamera.src = './assets/camera_light.svg';
});
