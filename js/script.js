...
const misGuifos = document.querySelector('#misGuifos');
...
const stringblock = document.getElementById('stringsearched');
...
const getSuggestString = async () => {
    if (inputBusqueda.value !== '') {
        const urlStringSuggest = 'https://api.giphy.com/v1/tags/related/' + inputBusqueda.value + '?api_key=' + apiKey;
...
const searchFunctions = async (stringSearch) => {
    ...
    try {
        ...
        titleSearched.innerText = `Resultados para "${stringSearch}":`
        createBlockTags(stringSearch);
        ...
        for (const iterator of dataGifSearchJON.data) {
            ...
            gif.setAttribute('src', iterator.images.original.url);
...
const getSuggestGif = async () => {
    for (let i = 0; i < 4; i++) {
        ...
        gif.setAttribute('src', dataGifJSON.data.images.original.url);
        gif.setAttribute('alt', dataGifJSON.data.title)
        gif.setAttribute('class', 'styleOfGifs')
...
getTrendingGif()
    .then((res) => {
        for (let i = 0; i < 25; i++) {
            ...
            gif.setAttribute('src', res.data[i].images.original.url);
            gif.setAttribute('class', 'styleOfGifs');
...
//FUNCION CREAR TAGS BUSCADOS

function createBlockTags(tag) {
    let block = document.createElement('div');
    stringblock.appendChild(block);
    block.setAttribute('class', 'blockTag font-family-white');
    block.innerText = '#' + tag;

    block.onclick = () => {
        searchFunctions(tag);
        inputBusqueda.value = tag
    }
}
